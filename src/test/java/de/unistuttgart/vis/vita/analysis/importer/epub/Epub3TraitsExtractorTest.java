...
public class Epub3TraitsExtractorTest {
  ...
  private List<List<String>> chaptersList = new ArrayList<List<String>>();
  ...
  public void testChaptersContent() {
    ...
    assertTrue(chaptersList.get(0).get(1).startsWith("Chapter 1. Loomings."));

    // first chapter: ending
    assertTrue(chaptersList.get(0).get(chaptersList.get(0).size() - 1)
        .endsWith("one grand hooded phantom, like a snow hill in the air."));

    // eleventh chapter: heading
    assertTrue(chaptersList.get(10).get(2).startsWith("Chapter 11. Nightgown."));

    // eleventh chapter: ending
    assertTrue(chaptersList.get(10).get(chaptersList.get(10).size() - 1)
        .endsWith("may prove in the mere skeleton I give."));

    // last chapter: heading
    assertTrue(chaptersList.get(chaptersList.size() - 1).get(2).startsWith("Epilogue"));
  ...
  public void testPartsContent() {
    ...
    assertTrue(parts.get(0).get(0).get(1).startsWith("Chapter 1. Loomings."));
    ...
    // first chapter: heading
    assertTrue(parts.get(1).get(0).get(1).startsWith("Chapter 2. The Carpet-Bag."));
    ...
    // last chapter: heading
    assertTrue(parts.get(1).get(parts.get(1).size() - 1).get(2).startsWith("Chapter 49. The Hyena"));
    ...
    // first chapter: heading
    assertTrue(parts.get(2).get(0).get(2).startsWith("Chapter 50. Ahab’s Boat and Crew. Fedallah."));
    
    // first chapter: ending
    assertTrue(parts.get(2).get(0).get(parts.get(2).get(0).size() - 1)
        .endsWith("indulged in mundane amours."));

    // last chapter: heading
    assertTrue(parts.get(2).get(parts.get(2).size() - 1).get(2).startsWith("Epilogue"));
